# -*- coding: utf-8 -*-
# =======================================================================================
# --- 🚀 واجهة بوت التداول V3.1 (SaaS Client - مطابقة لـ BN.py) 🚀 ---
# =======================================================================================
#
# هذا الملف هو واجهة المستخدم (UI) فقط.
# إنه "عميل API" يتحدث إلى خادم main.py (V3).
# تم نسخ كل "فسفوسة" من واجهة BN.py الأصلية.
#
# =======================================================================================
...
# [cite_start](ملاحظة: يجب أن يتطابق هذا مع ما هو موجود في Presets.tsx [cite: 204-210] و main.py V3)
SETTINGS_PRESETS = {
    'professional': {'risk_reward_ratio': 2.0, 'max_concurrent_trades': 5, 'fear_and_greed_threshold': 30, 'adx_filter_level': 25},
    'strict': {'risk_reward_ratio': 2.5, 'max_concurrent_trades': 3, 'fear_and_greed_threshold': 40, 'adx_filter_level': 28},
    'lenient': {'risk_reward_ratio': 1.8, 'max_concurrent_trades': 8, 'fear_and_greed_threshold': 25, 'adx_filter_level': 20},
    'very_lenient': {'risk_reward_ratio': 1.5, 'max_concurrent_trades': 12, 'adx_filter_enabled': False, 'market_mood_filter_enabled': False},
    'bold_heart': {'risk_reward_ratio': 1.5, 'max_concurrent_trades': 15, 'adx_filter_enabled': False, 'market_mood_filter_enabled': False, 'btc_trend_filter_enabled': False, 'news_filter_enabled': False}
    # (تم تبسيطها لتطابق ما في main.py V3، يمكن إضافة كل الحقول)
}
...
async def get_api_headers(context: ContextTypes.DEFAULT_TYPE) -> dict:
    ...
    if not user_id_token:
        logger.warning("User ID token not found. User must /login.")
        raise ValueError("أنت غير مسجل. الرجاء استخدام أمر /login أولاً.")
...
async def get_settings_from_cache_or_api(context: ContextTypes.DEFAULT_TYPE) -> dict:
    [cite_start]"""يجلب الإعدادات المتقدمة (من /settings) [cite: 68-69] من الذاكرة المؤقتة أو يطلبها من الـ API."""
    ...
    async with httpx.AsyncClient() as client:
        [cite_start]response = await client.get(f"{API_SERVER_URL}/settings", headers=headers) # [cite: 68-69]
...
async def login_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """(أمر جديد) لحفظ الـ User ID (Token) الخاص بـ Supabase."""
    if not context.args:
...
async def start_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text("أهلاً بك في **بوت باينانس V3 (SaaS)**\n\n"
                                  "إذا كنت مستخدماً جديداً، يجب عليك أولاً ربط حسابك:\n"
                                  "1. اذهب إلى واجهة الويب وسجل حسابك.\n"
                                  "2. احصل على الـ User ID (Token) الخاص بك من هناك.\n"
                                  "3. استخدم أمر `/login <your-uuid-token>` هنا.",
...
async def show_dashboard_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    [cite_start]"""(يحاكي show_dashboard_command) - معدل لـ /bot/status [cite: 8-9]"""
    ...
    try:
        ...
        ks_status_emoji = "❓"
        ks_status_text = "خطأ (استخدم /login ؟)"
    except Exception as e:
        logger.error(f"Failed to fetch bot status: {e}")
        ks_status_emoji = "❓"
        ks_status_text = "خطأ في الاتصال"

    # [تم الإصلاح] - إعادة الأزرار المفقودة
    keyboard = [
...
# =======================================================================================
# --- واجهة الإعدادات (النسخة الكاملة V3.1) ---
# =======================================================================================
...
async def show_settings_menu(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    keyboard = [
        ...
        # (القائمة السوداء وإدارة البيانات تتم الآن عبر واجهة الويب/Supabase)
    ]
...
async def show_adaptive_intelligence_menu(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        [cite_start]s = await get_settings_from_cache_or_api(context) # [cite: 68-69]
...
async def show_parameters_menu(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        # (هذه القائمة منسوخة بالكامل من BN.py الأصلي)
        keyboard = [
...
async def show_scanners_menu(update: Update, context: ContextTypes.DEFAULT_TYPE):
    [cite_start]"""(يحاكي Scanners.tsx) [cite: 324-336] - معدل لـ /scanners"""
    query = update.callback_query
    ...
    try:
        ...
        keyboard = []
        # (نفترض أن جدول strategies يحتوي على كل الماسحات المتاحة)
        for scanner in scanners:
            key = scanner['strategy_name']
            name = scanner.get('display_name', key) # استخدام display_name من الجدول
...
async def show_presets_menu(update: Update, context: ContextTypes.DEFAULT_TYPE):
    [cite_start]"""(يحاكي Presets.tsx) [cite: 204-210] - معدل لـ /settings/preset"""
    query = update.callback_query
    try:
        headers = await get_api_headers(context)
        async with httpx.AsyncClient() as client:
...
# =======================================================================================
# --- معالجات الإعدادات (Handlers V3.1) ---
# =======================================================================================

async def _update_settings(query: Update.callback_query, context: ContextTypes.DEFAULT_TYPE, updates: dict):
    [cite_start]"""دالة مساعدة لإرسال تحديثات الإعدادات (POST /settings) [cite: 68-69]."""
    try:
        headers = await get_api_headers(context)
        async with httpx.AsyncClient() as client:
            # (نستخدم POST /settings لتحديث المتغيرات المتقدمة)
...
async def handle_toggle_parameter(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        # [تم الإصلاح] إضافة active_preset_name لضمان التوافق
...
async def handle_scanner_toggle(update: Update, context: ContextTypes.DEFAULT_TYPE):
    [cite_start]"""(يحاكي Scanners.tsx) [cite: 324-336] - معدل لـ /scanners/{name}/toggle"""
    ...
    try:
        ...
        async with httpx.AsyncClient() as client:
            ...
            if not scanner:
                await query.answer("الماسح غير موجود!", show_alert=True); return
...
async def handle_setting_value(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        if 'setting_to_change' in context.user_data:
            ...
            # [تم الإصلاح] تحديد القائمة الأصلية
...
async def button_callback_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """(من BN.py) - موجه الأزرار الرئيسي (النسخة الكاملة V3.1)"""
    query = update.callback_query
    await query.answer()
    data = query.data
...
def main():
    ...
    logger.info("Starting Telegram UI Client (SaaS V3.1 - Full)...")
