...
logger = logging.getLogger("BotWorker_V2_1")
...
# [جديد] ذاكرة مؤقتة لمنع رسائل "توقف الفحص" المكررة
...
async def _manage_active_trade(trade: Dict, price: float):
    """ [تم الإصلاح] دالة مساعدة لـ "العيون": تدير الوقف المتحرك والإشعارات "التافهة". """
    ...
    # (هذا الإعداد غير موجود في advanced_variables، سنستخدم 2% كقيمة ثابتة)
    increment_pct = 2.0 
...
async def sync_cache_from_db():
    ...
    while True:
        try:
            ...
            if active_user_ids:
                async with db_utils.db_connection() as conn:
                    all_trades = await conn.fetch(
                        "SELECT * FROM trades WHERE status = 'active' AND user_id = ANY($1)", list(active_user_ids)
                    )
                for r in all_trades:
                    trade = dict(r)
            ...
            # مسح المخابئ للمستخدمين غير النشطين
            _clear_inactive_caches(active_user_ids)

            # مسح المخابئ للمستخدمين النشطين لإجبارهم على إعادة التحميل
...
async def _execute_close(user_id: UUID, trade: Dict, reason: str):
    """ (من BN.py) ينفذ أمر البيع الفعلي ويحدّث قاعدة البيانات. """
    ...
    if not exchange:
        ...
        await db_utils.set_trade_status(trade_id, 'active'); return # إعادة المحاولة
    try:
        ...
        if min_notional_str and (quantity_to_sell * close_price) < float(min_notional_str):
            ...
            closed_trade_data = await db_utils.close_trade(trade_id, close_price, 0.0) # إغلاق بخسارة 0
...
async def scan_for_user(user_id: UUID, all_tickers: Dict):
    """ [تم الإصلاح] ينفذ منطق الفحص الكامل لمستخدم واحد ويرسل إشعارات الحالة. """
    ...
    try:
        ...
        # 3. التحقق من الحد الأقصى للصفقات
        async with db_utils.db_connection() as conn:
        ...
        # (هذا المنطق يجب نقله من BN.py إلى هنا)
        if settings.market_mood_filter_enabled:
            # (منطق وهمي، يجب استخدام get_fear_and_greed_index)
            fng = 50
        ...
        # --- [الإصلاح] إرسال إشعار نجاح الفحص ---
        ...
        if user_id in SCAN_SKIP_NOTIFICATION_CACHE:
            del SCAN_SKIP_NOTIFICATION_CACHE[user_id] # مسح ذاكرة التخطي
...
async def _notify_scan_skip(user_id: UUID, reason: str):
    """(جديد) يرسل إشعار تخطي الفحص مرة واحدة فقط."""
...
async def _execute_buy(exchange: ccxt.Exchange, user_id: UUID, signal: dict, settings: TradingVariables) -> bool:
    ...
    # [تصحيح] استخدام min_trade_amount من الإعدادات
    ...
    try:
        ...
        if new_trade:
            ...
            # --- [الإصلاح] إرسال إشعار فتح الصفقة ---
            await db_utils.create_notification(
...
async def main():
    logger.info("--- 🚀 Bot Worker (SaaS Engine V2.1 - Full Notifications) Starting Up... ---")
